...
* NEWICK: Newick format is a commonly used
way of representing tree topologies as text. Put simply, monophyletic clades are surrounded
by parentheses and sister clades are separated by commas. For example, a simple tree could
be written as (((A,B),C),(D,E)). Newick format also contains information about branch
lengths (after colons) and node names (after closed parentheses).
...
dna <- read.dna(file = "~/EEB_Program/Courses/Reproducible_Science/Chapter_10/phylogenies/vanilla_seqs_specs.fasta", format = "fasta")
...
tree <- read.tree("~/EEB_Program/Courses/Reproducible_Science/Chapter_10/phylogenies/Theobroma_tree.nwk")
str(tree)
class(tree)
