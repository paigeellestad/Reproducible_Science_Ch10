...
### Example 2: Additional Customization Using ggplot2
...
ggplotgui::ggplot_shiny(GoodDogs) #this opens a shiny app to point and click make a ggplot
```

We can copy the code that the ggplot GUI app uses to produce this histogram as a shortcut. I also copied the code for a ratings histogram. Because the app is limited in features, we will work on customizing these figures more next
```{r Code from GUI}
...
#copied ratings histogram, changed df to GoodDogs and saved as object
DogRating <- ggplot(GoodDogs, aes(x = Rating)) +
...
## Exercise 1A: Producing a figure like this

## Exercise 1B: Producing and arranging two figures like this
