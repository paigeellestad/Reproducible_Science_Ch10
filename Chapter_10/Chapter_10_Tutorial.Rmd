...
```{r packages2, eval = FALSE, echo=FALSE, warning=FALSE, include=FALSE}
...
### Reading Newick files and visualizing more trees
...
```{r newick_info, results='hide', include = TRUE}
...
### Using ggtree to plot phylogenetic trees
...
```{r vanilla_tree, include=TRUE}
tree <- read.tree("vanilla.nwk")
ggtree(tree)
```

Add a scale.
```{r vanilla tree scale, include=TRUE}
# add a scale
...
```{r vanilla tree remove branch lengths, include = TRUE}
ggtree(tree, branch.length="none")

ggtree(tree, branch.length="none", color="blue", size=2, linetype=3)#
```

```{r vanilla tree plot, include = TRUE}
...
[1] D. Watts and S. Strogatz. Collective dynamics of ’small-world’ networks. Nature, 393(6684):440–442,
1998. [p28]
