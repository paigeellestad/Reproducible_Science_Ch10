...
#you will need to have all of these packages installed and loaded to complete this tutorial 
...
# Exercise 2: Visualizing Networks
...
```{r packages2, echo=FALSE, warning=FALSE, include=FALSE}
...
# network with grey nodes and red edges
ggnet2(net, size = 6, color = "grey", edge.size = 1, edge.color = "red")

# possible modifications to node/edge size
#big nodes and edges
ggnet2(net, size = 8, color = "grey", edge.size = 3, edge.color = "red")

#small nodes and big edges
ggnet2(net, size = 2, color = "grey", edge.size = 4, edge.color = "red")

#small nodes and edges
ggnet2(net, size = 1, color = "grey", edge.size = 1, edge.color = "red")
```
...
```{r solution2B, echo = FALSE, eval = FALSE}
...
```{r solution2C, echo = FALSE, eval = FALSE}
# using indegree
ggnet2(net, size = "indegree")

#using outdegreee
ggnet2(net, size = "outdegree")
```
...
```{r shapes, echo = TRUE, eval = TRUE, warning = FALSE}
...
```{r solution2D, echo = FALSE, eval = FALSE}
# solution 
...
```{r solution2E, echo = FALSE, eval = FALSE}
# solution
ggnet2(net, size = 12, label = TRUE, color = "tomato", label.size = 4, label.color = "steelblue", label.alpha = 0.5)
```
...
- ggnet2 Tutorial
  ...
  - https://exts.ggplot2.tidyverse.org/ggnetwork.html 
- Network studies
  - Community detection in networks (Girvan and Newman, 2002)
  - Node failure in interdependent networks (Buldyrev et al., 2010)
  - Neural network of C. elegans (watts and Strogatz, 1998)
